...
describe("DSProxyFactory_fixed", function () {
	it("Fixed proxy factory", async function () {
		...
		const DSProxy = await ethers.getContractFactory("DSProxy_fixed", addr1);
		...
		await proxy.connect(addr1).executeCode(bytecode, calldata); // Here we "fixed" a culprit
